;;; anathemacs-theme-faces.el --- Modify fonts and sizing here to maintain

;;; Commentary:
;;; Code:

;; FACES: maintain consistency and ease customization across various screen resolutions and font families

(defface mono-xs '((t :family "Victor Mono" :height 85)) "xs monospace font")
(defface mono-sm '((t :family "Victor Mono" :height 95)) "sm monospace font")
(defface mono-md '((t :family "Victor Mono" :height 105)) "md monospace font")
(defface mono-lg '((t :family "Victor Mono" :height 130)) "lg monospace font")
(defface mono-xl '((t :family "Victor Mono" :height 160)) "xl monospace font")

;; (defface mono-xs '((t :family "PragmataPro" :height 80)) "xs monospace font")
;; (defface mono-sm '((t :family "PragmataPro" :height 90)) "sm monospace font")
;; (defface mono-md '((t :family "PragmataPro" :height 100)) "md monospace font")
;; (defface mono-lg '((t :family "PragmataPro" :height 125)) "lg monospace font")
;; (defface mono-xl '((t :family "PragmataPro" :height 150)) "xl monospace font")

;; (defface sans-xs  '((t :family "NotoSans" :height 85)) "xs sans serif font")
;; (defface sans-sm  '((t :family "NotoSans" :height 95)) "sm sans serif font")
;; (defface sans-md  '((t :family "NotoSans" :height 110)) "md sans serif font")
;; (defface sans-lg  '((t :family "NotoSans" :height 160)) "lg sans serif font")
;; (defface sans-xl  '((t :family "NotoSans" :height 220)) "xl sans serif font")
;; (defface sans-xxl '((t :family "NotoSans" :height 300)) "xxl sans serif font")

(defface sans-xs  '((t :family "Avenir" :height 80)) "xs sans serif font")
(defface sans-sm  '((t :family "Avenir" :height 90)) "sm sans serif font")
(defface sans-md  '((t :family "Avenir" :height 110)) "md sans serif font")
(defface sans-lg  '((t :family "Avenir" :height 120)) "lg sans serif font")
(defface sans-xl  '((t :family "Avenir" :height 220)) "xl sans serif font")
(defface sans-xxl '((t :family "Avenir" :height 300)) "xxl sans serif font")

;; (defface serif-xs  '((t :family "ETBookOT" :height 90)) "xs serif font")
;; (defface serif-sm  '((t :family "ETBookOT" :height 110)) "sm serif font")
;; (defface serif-md  '((t :family "ETBookOT" :height 130)) "md serif font")
;; (defface serif-lg  '((t :family "ETBookOT" :height 150)) "lg serif font")
;; (defface serif-xl  '((t :family "ETBookOT" :height 240)) "xl serif font")
;; (defface serif-xxl '((t :family "ETBookOT" :height 320)) "xxl serif font")

(defface serif-xs  '((t :family "AdobeGaramondPro" :height 90)) "xs serif font")
(defface serif-sm  '((t :family "AdobeGaramondPro" :height 110)) "sm serif font")
(defface serif-md  '((t :family "AdobeGaramondPro" :height 130)) "md serif font")
(defface serif-lg  '((t :family "AdobeGaramondPro" :height 150)) "lg serif font")
(defface serif-xl  '((t :family "AdobeGaramondPro" :height 240)) "xl serif font")
(defface serif-xxl '((t :family "AdobeGaramondPro" :height 320)) "xxl serif font")

;;; anathemacs-theme-faces.el ends here
